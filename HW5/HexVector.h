...
class HexVector : public AbstractHex{
	public:
		...
		inline int numberOfMoves()					override{return markedCell4Board_Vector;}
		void reset()								override;
		inline void increaseMarkedCell4Board() {markedCell4Board_Vector++;}
		inline HexVector()							{size=6; selection=1; createBoard();}
		inline HexVector(int tempSize)				{size=tempSize; selection=1; createBoard();}
		inline HexVector(int tempSize,int againstWho)	{size=tempSize; selection=againstWho; createBoard();}
		...
		void playGame();
		void continuedGame();
		bool compare(HexVector& other)const;
		
	private:
		void resizeBoard(vector<vector<Cell> > HexVector);
		void printBoard();
