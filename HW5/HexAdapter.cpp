...
string HexAdapter<T>:: last_word(string& str)const{
...
template<template<typename...> class T>
void HexAdapter<T>:: convert2upper_case(int picked){
	enumType tempPath;
...
template<template<typename...> class T>
void HexAdapter<T>:: saving(const string& file_name,int whoseTurn,int againstWho){
	ofstream myfile;
...
template<template<typename...> class T>
void HexAdapter<T>:: loading(const std::string& file_name,int whoseTurn, int aganistWho){
	int i;
	char fromFile;
	row=0,col=0;
...
template<template<typename...> class T>		
int HexAdapter<T>:: randomGenerator(){
	int rand;
...
template<template<typename...> class T>
void HexAdapter<T>:: continuedGame(){
	while(1){
		printBoard();
		//cout<<*this;
...
template<template<typename...> class T>
void HexAdapter<T>:: play(int row,int col,enumType who){
...
template<template<typename...> class T>
bool HexAdapter<T>:: find_path_x(int i,int j,int flag){
	if(j==size){
...
template<template<typename...> class T>
bool HexAdapter<T>:: find_path_o(int i,int j,int flag){
...
template<template<typename...> class T>
bool HexAdapter<T>:: compare(HexAdapter<T>& other)const{
	if(getCompareMarked()>other.getCompareMarked()) return true;
	else return false;
}
template<template<typename...> class T>
void HexAdapter<T>:: resizeBoard(T <T<Cell>> hexCells){
...
template<template<typename...> class T>
void HexAdapter<T>:: reset(){									//reset function for vector
	for(int i=0;i<size;i++){
		for(int j=0;j<size;j++){
...
template<template<typename...> class T>
AbstractHex:: Cell HexAdapter<T>:: operator ()(int r,int c){
	/*if(r<1||r>getSize()||c<1||c>getSize())
		throw outOfRange();
	else*/ return hexCells[r][c];
}
template<template<typename...> class T>
bool HexAdapter<T>:: operator ==(AbstractHex& game2){
