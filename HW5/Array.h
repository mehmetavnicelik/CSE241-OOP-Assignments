...
class HexArray : public AbstractHex{
	public:
		...
		inline HexArray()							{size=6; selection=1; createBoard();}
		inline HexArray(int tempSize)				{size=tempSize; selection=1; createBoard();}
		inline HexArray(int tempSize,int againstWho)	{size=tempSize; selection=againstWho; createBoard();}
		...
		void playGame();
		void continuedGame();
		bool compare(HexArray& other)const;
		
	private:
		void resizeBoard(vector<vector<Cell> > HexArray);
		void printBoard();
